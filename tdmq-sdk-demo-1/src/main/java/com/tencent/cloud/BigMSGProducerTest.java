...
//        String SUB_NAME = args[3];
        ...
        if (messageContentType == 1){
            ...
            randomMessage = sb.toString();
        }

        StringBuilder sb = new StringBuilder(msgSize*1024);
        for(int i = 0; i < msgSize; ++i) {
            String message = messageContentType == 0 ? raw : randomMessage;
            sb.append(message);
        }
        String all = sb.toString();
        System.out.printf("chunk打开：%d 压缩打开：%d 大小：%d",chunk,compre,msgSize);
        System.out.println("消息大小为:"+all.length()/1024/1024+"MB");
        System.out.println("压缩算法:"+zipTypeMap.get(zipType).toString());
        for(int i = 0; i < msgCount; ++i) {
            TimeUnit.MILLISECONDS.sleep(sleepms);
            Long start = System.currentTimeMillis();
            TypedMessageBuilder<byte[]> value = producer.newMessage().value(all.getBytes());
