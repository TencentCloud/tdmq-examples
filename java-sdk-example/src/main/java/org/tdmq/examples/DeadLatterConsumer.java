...
public class DeadLatterConsumer {
    public static void main(String[] args) {
        ...
        final String deadLetterTopicName = "persistent://your-tenant/your-ns/your-topic-DLQ";
        final String retryLetterTopicName = "persistent://your-tenant/your-ns/your-topic-RETRY";
        ...
        try {
            ...
            /**
             * 创建 []byte 类型的 Consumer，同时开启重试队列和死信队列的功能
             *
             * 参数说明：
             *     enableRetry: 开启重试队列的功能，默认为 false
             *          当开启重试队列的功能时，需要同时在 deadLetterPolicy 中指定重试队列的 Topic，
             *          也可以在创建原始 Topic 的时候在控制台开启是否自动创建死信重试 Topic。
             ...
             *     deadLetterPolicy: 配置死信策略，即当消息被重复推送指定次数之后，当前消息会在原始 Topic 中被确认然后转存到死信 Topic 中
             *          maxRedeliverCount: 指定消息最多被推送的次数
             *          deadLetterTopic: 指定死信队列的 Topic 的名字，可以手动指定，也可以在创建原始 Topic 的时候在控制台开启自动创建死信 Topic。
             *
             */
            Consumer<byte[]> consumerWithRetry = pulsarClient.newConsumer()
                    .topic(topicName)
                    .enableRetry(true)
                    ...
                    .deadLetterPolicy(DeadLetterPolicy.builder()
                            ...
                            .retryLetterTopic(retryLetterTopicName)
                            .build())
                    .subscribe();

            /**
             * 创建 []byte 类型的 Consumer，只开启死信队列的功能。
             *
             * 参数说明：
             *     subscriptionName: 可以从管控台创建，也可以在代码中指定。如果代码中指定即 Broker 会自动创建该订阅
             *     subscriptionType: 订阅的类型, 当前 TDMQ 支持如下订阅类型
